...
// Options는 audit author와 event clock을 설정한다.
type Options struct {
	Author string
	Now    func() time.Time
}

// Service는 현재 order state를 갱신하기 전에 immutable audit entry를 append한다.
type Service struct {
	mu     sync.Mutex
...
// NewService는 audit-backed order history service를 만든다.
...
// Confirm은 pending order를 confirmed로 이동한다.
...
// Ship은 confirmed order를 shipped로 이동한다.
...
// Cancel은 pending 또는 confirmed order를 cancelled로 이동한다.
...
// Current는 order 하나의 current-state projection을 반환한다.
...
// History는 order aggregate 하나의 complete demo history를 반환한다.
...
// Find는 bounded order-only audit query result를 반환한다.
