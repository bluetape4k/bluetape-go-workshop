...
import (
	...
	"github.com/bluetape4k/bluetape-go-workshop/examples/token-refresh-claims/internal/tokenrefresh"
...
func main() {
	...
	service, err := tokenrefresh.NewService()
	if err != nil {
		log.Fatalf("create token refresh claims service: %v", err)
	}
	server := newHTTPServer(addr, tokenrefresh.NewRouter(service))
	...
	go func() {
		log.Printf("token refresh claims example listening on http://%s", addr)
