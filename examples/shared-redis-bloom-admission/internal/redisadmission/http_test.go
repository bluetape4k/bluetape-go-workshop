...
func TestRouterReturnsStats(t *testing.T) {
	...
	if stats.ExpectedInsertions != 500 || stats.ApproximateElementCount == 0 || !stats.AuthoritativeStoreRequired {
		t.Fatalf("stats = %+v, want configured non-empty shared filter stats", stats)
	}
}
...
func newTestRouter(t *testing.T) http.Handler {
	...
	return NewRouter(newTestService(ctx, t, client, "http-instance", namespace(t), 500))
}
