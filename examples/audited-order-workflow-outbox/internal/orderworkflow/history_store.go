...
// HistoryStore 는 불변 감사 엔트리를 영속화하고 audit.HistoryReader를 구현한다.
type HistoryStore struct {
	db *sql.DB
}

// DeliveryStatus 는 이벤트 내용을 노출하지 않고 outbox 상태를 집계한다.
...
// NewHistoryStore 는 영속 이력 조회를 db에 바인딩한다.
...
// Insert 는 호출자가 소유한 트랜잭션 또는 세션을 통해 entry를 기록한다.
...
// Find 는 안정적인 revision 순서 또는 전역 position 순서로 엔트리를 반환한다.
...
// FindByCommandID 는 표준 이벤트 ID 또는 idempotency identity에 해당하는 엔트리를 찾는다.
...
// DeliveryStatus 는 짧은 제한 시간 안에서 outbox 집계 진단값을 반환한다.
...
// LoadHistory 는 aggregate의 검증된 감사 이력을 재구성한다.
...
// Latest 는 aggregate에 대해 영속화된 가장 높은 revision 엔트리를 반환한다.
...
// LatestSnapshot 은 스냅샷 엔트리가 있으면 가장 최신 항목을 반환한다.
...
// PreviousSnapshot 은 제외 상한 revision 이전의 가장 최신 스냅샷을 반환한다.
