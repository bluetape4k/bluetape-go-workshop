...
func TestResolveHTTPAddr(t *testing.T) {
	for _, value := range []string{"", "localhost:9090", "127.0.0.1:9090", "[::1]:9090"} {
	...
	for _, value := range []string{":8080", "0.0.0.0:8080", "[::]:8080", "192.168.1.10:8080", "audit.example:8080"} {
	...
	for _, value := range []string{"127.0.0.1", "127.0.0.1:0", "127.0.0.1:65536", "127.0.0.1:nope"} {
		if _, err := resolveHTTPAddr(value, true); err == nil {
			t.Fatalf("resolveHTTPAddr(%q) accepted invalid address", value)
		}
	}
}
...
func TestNewHTTPServer(t *testing.T) {
	...
	if server.Addr != "127.0.0.1:8080" || server.ReadHeaderTimeout != 2*time.Second || server.ReadTimeout != 5*time.Second || server.WriteTimeout != 5*time.Second || server.IdleTimeout != 30*time.Second || server.MaxHeaderBytes != 1<<20 {
...
func TestRunServerGracefulCancellation(t *testing.T) {
	...
	if server.shutdownCalls != 1 || server.closeCalls != 0 || !server.sawFiveSecondDeadline {
		t.Fatalf("shutdown=%d close=%d deadline=%v", server.shutdownCalls, server.closeCalls, server.sawFiveSecondDeadline)
	}
}
...
func TestRunServerForcesCloseAndJoinsErrors(t *testing.T) {
	...
	server.closeErr = errors.New("close secret failed")
	server.afterStopErr = errors.New("listen secret failed")
	ctx, cancel := context.WithCancel(context.Background())
	...
	if strings.Contains(logs.String(), "secret") || !strings.Contains(logs.String(), "event=forced_close") {
