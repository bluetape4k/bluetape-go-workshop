...
| [`examples/gin-sql-order-service`](examples/gin-sql-order-service) | [English](examples/gin-sql-order-service/README.md) \| [한국어](examples/gin-sql-order-service/README.ko.md) | Milestone Gin order service that composes HTTP parsing, service-owned SQL transactions, multi-table repositories, item updates, and rollback proof. | `sqlkit`, `gin`, `testcontainers/postgres` |
...
| [`examples/gin-content-moderation-workflow`](examples/gin-content-moderation-workflow) | [English](examples/gin-content-moderation-workflow/README.md) \| [한국어](examples/gin-content-moderation-workflow/README.ko.md) | Runnable Gin workflow for multilingual routing, masking, inspectable records, and bounded POST JSON search. | `gin`, `textsearch/language`, `textsearch/japanese` |
...
## Run the SQL Order Repository Example
...
The example persists an order, reads it by primary key, lists customer orders by
status, and documents how `sqlkit` complements direct `database/sql` without
becoming an ORM or migration owner.
...
## Run the SQL Transaction Boundary Example
...
The example places an order inside one explicit service-owned transaction,
debits product stock, inserts order rows, and proves rollback on stock conflict,
payment rejection, and context cancellation.
...
## Run the Gin SQL Order Service Example

Print the no-database order service preview:

```bash
go run ./examples/gin-sql-order-service
```

The example composes the SQL repository, SQL transaction boundary, and Gin CRUD
lessons. Gin owns HTTP parsing and public errors, the service owns
`sqlkit.WithTx`, and repositories own the order header, item, and status-event
SQL. A deterministic `reject_after_items` request proves rollback after item
inserts without adding payment, inventory, fulfillment, or outbox scope.

Run the HTTP service against PostgreSQL by setting `DATABASE_URL`. The example
creates its local demo tables on startup, while production services should move
schema changes into a migration owner.
...
Run the PostgreSQL-backed handler, service, and repository tests:

```bash
go test -count=1 ./examples/gin-sql-order-service/...
```

## Run the Transactional Outbox Publisher Example

Start PostgreSQL and Redis, then provide their endpoints to commit one order
and its outbox event before relaying the committed event to Redis Streams:
...
The [example README](examples/transactional-outbox-publisher/README.md) includes
pinned container commands, expected JSON and stream fields, rerun identities,
race tests, and production at-least-once boundaries.

Run the PostgreSQL and Redis Testcontainers suite:
...
## Run the SQS Floci Worker Example
...
The example models enqueue, receive, handler success acknowledgement, and
handler failure retry visibility while documenting SQS at-least-once delivery
and handler idempotency requirements.

Run the deterministic worker tests:

```bash
go test -count=1 ./examples/sqs-floci-worker/...
```

Run the optional Floci SQS smoke test serially when Docker is available:
...
## Run the Gin Text Search Service Example
...
Run the local Gin API and try the search/mask endpoint:
